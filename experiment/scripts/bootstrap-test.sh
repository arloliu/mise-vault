...
echo "== 1. documented bootstrap: git clone (netrc) + sh install.sh =="
export MISE_VAULT_REPO_URL="$GITLAB/devtools/mise-vault.git" MISE_VAULT_REF="$REF"
rm -rf "$T/bootstrap" \
...
grep -q "nexus_url=$NEXUS/repository/devtools" "$T/.config/mise/conf.d/mise-vault.toml" 2>/dev/null \
  && ok "aliases carry nexus_url option" || bad "nexus_url in aliases"
