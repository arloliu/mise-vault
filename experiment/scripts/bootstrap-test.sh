...
echo "== 6. branch clone installs the cloned commit — and that commit is the code under review =="
...
# the suite's pass counts only vouch for the code under review if the
# experiment GitLab default branch IS the local HEAD — otherwise fail loudly
CLONED=$(git -C "$T/bootstrap-main" rev-parse HEAD 2>/dev/null || echo none)
[ "$CLONED" = "$LOCAL_HEAD" ] \
