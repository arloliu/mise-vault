...
## 20. Ecosystem tool types, Phase A — npm and pypi (2026-08-20)

A third and fourth tool type landed: `npm` and `pypi`, installed by the
ecosystem's own package manager (npm for npm; pipx by default or uv
opt-in for pypi) instead of a Nexus artifact or a go-proxy build.
prettier (npm) and ruff (pypi) are the seeded examples, versions 3.9.6
and 0.16.3.
The design is fully recorded in
`docs/specs/2026-08-20-ecosystem-tools-design.md`
(and its companion handoff); that spec file remains the authoritative
detail for field grammars, install semantics, and the environment table
— this entry records what shipped and what changed during
implementation, not a restatement of the design.
poc-test: 67/67.

| # | Decision |
|---|---|
| D27 | **The network principle is scoped, not weakened, for these two types.** Artifact and go-installed tools keep the absolute rule unchanged: the plugin constructs every URL itself, from Nexus or the plugin-controlled go proxy, and never falls back. npm and pypi tools are the deliberate exception the design called for: the plugin constructs no registry URL at all for them — the selected runner reads whatever registry the user's own environment already configures (`.npmrc` for npm; `pip.conf` for pipx, since it installs through pip; `uv.toml` or its env-var equivalents for the opt-in uv runner) — so egress is a network-policy and user-configuration matter there, the same as an unmanaged `npm install` or `pip install` already is. AGENTS.md, README.md, and docs/design.md were all amended to state this scoping explicitly everywhere they previously stated the rule as absolute. |
| D28 | **`versions.schema.json` uses `anyOf`, not `oneOf`, for its per-item shapes — a deviation from the original plan, accepted by the project owner.** Bare-version records for go, npm, and pypi overlap: a string like `"1.0.0"` legally matches more than one type's version envelope at once, and a single JSON Schema instance has no way to see the sibling `tool.json` to know which type actually applies to a given record. `oneOf` would then reject some valid records as ambiguous (matching more than one branch) purely as a schema artifact, unrelated to whether the record is actually wrong. The schema therefore only proves shape-membership — "this record is a valid go-or-npm-or-pypi-shaped bare version" — and `scripts/validate-catalog` is the engine that enforces the EXACT grammar for the tool's declared type, cross-referencing `tool.json`, which a schema alone cannot do. |
| D29 | **When `bin` is absent, the tool name itself must satisfy the bin grammar — including its 64-character cap — since `bin` defaults to the tool name.** Discovered during implementation: an npm or pypi tool with a long `name` and no explicit `bin` would otherwise pass catalog validation and only fail at install time, when the plugin tries to verify a binary whose (defaulted) name violates the grammar it never checked. `scripts/validate-catalog` now enforces this at approval time — a tool whose name is too long for the bin grammar must add an explicit `bin` field, and validation fails with that message rather than silently deferring the problem to whoever runs `mise install` first. |
| D30 | **Three empirical findings from running the real tooling.** (1) `PIPX_HOME`/`PIPX_BIN_DIR` place a pipx 1.4.3 installation exactly where told, but `PIPX_MAN_DIR`, `PIPX_COMPLETION_DIR`, `PIPX_DEFAULT_BACKEND`, and `PIPX_FETCH_PYTHON` are silently inert on that version — each was introduced in a later pipx release, so the no-leftover-files and pinned-backend guarantees these variables provide only hold on a sufficiently recent pipx. The plugin sets all of them unconditionally regardless, since they are forward-compatible and an old pipx just ignores the ones it does not recognize. (2) A plugin hook's `print()` output is visible in default `mise install` output, surfacing as `INFO [vault] ...` — verified against the experiment stack — which is how the npm/pypi branches' effective-registry diagnostic line (`npm config get registry`, display-only, never validated) reaches an ordinary developer with no extra flag. (3) Nexus roles created by provisioning must be create-or-update: `devtools-read` was create-once, so adding the new npm-proxy/pypi-proxy read privileges to `provision-nexus.sh` did not propagate to a role that already existed on a previously provisioned stack; the script was fixed to refresh the role's privileges on every run, and anyone who provisioned before these repos existed needs one re-run to pick them up. |

---

## 21. Ecosystem tool types, Phase B — cargo (2026-08-20)

A fifth tool type landed: `cargo`, installed by shelling out to
`cargo install <crate> --version <version> --locked --root
<install_path>` against whatever crates.io mirror the user's own
`~/.cargo/config.toml` already points at, instead of a Nexus artifact,
a go-proxy build, or an npm/pypi runner.
tokei is the seeded example, version 14.0.0 — the latest stable,
non-yanked release on crates.io at approval time.
Unlike npm and pypi, a cargo tool has no runner-selection environment
variable at all: cargo is the only tool that can build a crate from
source, so it is always the runner.
The design is fully recorded in
`docs/specs/2026-08-20-ecosystem-tools-design.md`
(the same spec that covered Phase A); that spec file remains the
authoritative detail for field grammars, install semantics, and the
config.toml resolution ladder — this entry records what shipped and
what changed during implementation, not a restatement of the design.
poc-test: 78/78.

| # | Decision |
|---|---|
| D31 | **A yanked crate version is treated as absent, not as a distinct rejection reason.** `scripts/add-version`'s cargo probe parses the sparse index's JSON-lines format and only accepts a line whose `vers` matches the requested version AND whose `yanked` field is not `true`; a version that exists in the index only under `"yanked": true` is reported the same way as a version missing entirely. This is deliberate, not an oversight: crates.io yanking is itself crates.io's fail-closed signal that a version should not be installed (`cargo install` refuses a yanked version outright), so refusing it at approval time — before it ever reaches `versions.json` — keeps the catalog's own fail-closed posture consistent with the registry's. |
| D32 | **An unresolvable `[source.crates-io] replace-with` in the user's cargo configuration is a hard error, never a silent fallback to the public index.** `scripts/add-version` reads `~/.cargo/config.toml` (or `$CARGO_HOME/config.toml`, which replaces the default path outright — matching cargo's own `CARGO_HOME` precedence) the same way `cargo install` itself would: a `replace-with` naming a registry with no matching `[registries.<name>]` `index` aborts with a message naming the fix, rather than quietly reaching for `index.crates.io`. A `[source.crates-io]` section with no `replace-with` at all is not an error — it means the default public index legitimately applies, mirroring cargo's own default — but a *broken* redirect must never resolve to "try the public internet instead," which would be exactly the kind of fallback this project forbids. Only a `sparse+http(s)://` index is accepted; a git-index registry is refused with its own message, since the probe (and cargo's own sparse-index install path) cannot read a git-backed index the same way. |
| D33 | **The test image installs a rustup-managed stable Rust toolchain instead of Ubuntu 24.04's apt `rustc`/`cargo` (1.75.0), which is empirically too old to build current crates** (`cargo install tokei --version 14.0.0 --locked` fails: a dependency, `clap-cargo`, requires rustc 1.86 or newer). The rustup toolchain (verified: stable 1.97.1) builds tokei in well under a minute against a warm proxy cache. `gcc` plus an explicit `libc6-dev` (not the much larger `build-essential`) is enough for the one C linker cargo needs; the image installs with `--no-install-recommends`, and `gcc` only recommends `libc6-dev` rather than depending on it, so omitting it fails linking on a missing `crt1.o`/`crti.o`. |
| D34 | **`CARGO_HOME` must stay unset in the test image, or cargo silently stops reading the user's `~/.cargo/config.toml` registry channel.** Verified empirically: with `CARGO_HOME` pinned to a fixed system directory (the pattern used for `RUSTUP_HOME`, which correctly holds system-level toolchain-selection state), `cargo install` ignored a `~/.cargo/config.toml` written under a throwaway test `$HOME` entirely and reached for `index.crates.io` directly instead of the configured Nexus proxy. `RUSTUP_HOME` and `PATH` (for the rustup-installed shim binaries) are pinned system-wide; `CARGO_HOME` is left to default to `$HOME/.cargo` at runtime, so the per-test throwaway `$HOME` and its registry configuration are exactly what cargo reads — the same discovery cargo itself documents. |
